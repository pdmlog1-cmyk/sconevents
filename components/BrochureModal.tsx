...
type Variant = 'brochure' | 'scientific_program';
type Props = {
  variant?: Variant;
  /** Trigger element content (e.g. text + icon) */
  children: ReactNode;
...
const COPY: Record<Variant, { title: string; subtitle: string; cta: string }> = {
  brochure: {
    title: 'Download the conference brochure',
    subtitle: 'Tell us a little about yourself and we\'ll send the brochure straight to your inbox — and unlock the download instantly.',
    cta: 'Send & Download Brochure',
  },
  scientific_program: {
    title: 'Download the scientific program',
    subtitle: 'Share your details to download the latest scientific program PDF.',
    cta: 'Send & Download Program',
  },
};

type FieldKey = 'first_name' | 'email' | 'phone' | 'country' | 'captcha';

export default function BrochureModal({ variant = 'brochure', children, className, style }: Props) {
  ...
  // We need access to document.body for createPortal. Defer until after mount.
  useEffect(() => { setMounted(true); }, []);

  // Auto-open if the URL says so. Used by the landing page's "Download Brochure"
  // button which links to `?brochure=open` on the main site. A window-level
  // flag guards against multiple BrochureModal instances all opening at once.
  useEffect(() => {
    try {
      const params = new URLSearchParams(window.location.search);
      const want = variant === 'brochure' ? 'brochure' : variant === 'scientific_program' ? 'program' : null;
      if (!want) return;
      if (params.get(want) !== 'open') return;
      const flag = `__autoOpened_${want}` as const;
  ...
  // Lock body scroll + ESC key handling while open
  useEffect(() => {
  ...
  function getCaptchaToken(): string {
    const resp = formRef.current?.querySelector<HTMLInputElement>('[name="h-captcha-response"]');
    return resp?.value || '';
  }
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      try {
        ...
        a.click();
        a.remove();
      } catch { /* user can still use the manual link in the success view */ }
    } catch (e) {
  ...
  const modalNode = (
    <div className={`lead-modal${open ? ' open' : ''}${success ? ' sent' : ''}`} role="dialog" aria-hidden={!open}>
